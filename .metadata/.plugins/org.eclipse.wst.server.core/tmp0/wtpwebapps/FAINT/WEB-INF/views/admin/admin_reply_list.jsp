                ...
                <div class="col-md-12 col-sm-12 col-xs-12 table-responsive">
                    ...
                    <table id="posts-table" class="table table-tools table-striped">
                       <thead>
						...
						<tbody>
							<c:forEach items="${ARList}" var="ReplyVO">
								<tr>
									<td><input name="check" type="checkbox" value="${ReplyVO.replyid}"></td>
									<td><a href="/member/${ReplyVO.nickname}">${ReplyVO.nickname}</a></td>
									<td><a href="/member/${ReplyVO.nickname}">${ReplyVO.email}</a></td>
									<td>${ReplyVO.comment}</td>
									<td><fmt:formatDate value="${ReplyVO.regdate}" type="both" pattern="yy년 MM월 dd일 HH시mm분ss초"/></td>
									<td id = "postRead" onclick = "postRead(this)"><button  value = "${ReplyVO.postid}">게시물 상세보기</button></td>
								</tr>
							</c:forEach>
						</tbody>
					</table>
...
$(".pagination li a").on("click", function(event) {
	...
	var jobForm = $("#jobForm");
		...
		jobForm.attr("action", "<c:url value='/admin/post'/>").attr("method", "get");
		jobForm.submit();
});
