...
/* Modal Content */
.followModal-content, .reportModal-content {
...
<script id="modalReport" type="text/x-handlebars-template">
<div id="myReportModal" class="reportModal">
	<span class="close">&times;</span>
	<div class="reportModal-content">

 		<div class="reportitle"><strong>유저 신고</strong></div>
			<hr class = "reportLine>
        <div class="box-body">
            <div class="form-group">
                <label for="title">제목</label> <input type="text" id = "title" class="form-control" placeholder="제목을 입력해주세요.">
            </div>
			<div class="form-group">
                <label for="category">신고 유형</label>
					<select class="form-control" id = "category">
						<option value="0">신고 유형을 선택해주세요.</option>
                		<option value="1">스팸</option>
                		<option value="2">음란물</option>
                		<option value="3">편파적 발언 및 상징</option>
                		<option value="4">폭력 또는 폭력 위협</option>
						<option value="5">마약 판매 및 홍보</option>
						<option value="6">괴롭힘 및 따돌림</option>
						<option value="7">사칭</option>
						<option value="8">지적 재산권 침해</option>
						<option value="9">자해</option>
						<option value="10">미성년자</option>
                </select>
            </div>
            <div class="form-group">
                <label for="nickname">신고 대상</label> <input type="text" id = "nickname" class="form-control" value = "{{nickname}}" readonly>
            </div>
            <div class="form-group">
                <label for="writer">작성자</label> <input type="text" id = "writer" class="form-control" value="{{writer}}" readonly>
            </div>
            <div class="form-group">
                <label for="caption">신고 내용</label>
                <textarea class="form-control" id = "caption" rows="3" placeholder="신고 내용을 입력해주세요."></textarea>
            </div>

        	<div class="box-footer">
            	<button class="report" id = "report" onclick="reportRegister()">보내기</button>
...
$("#moreAction").on("click",function(){	   
	...
	var list = '<li class="_o2wxh"><button class="_h74gn" id="btnUserReport" onclick="userReport()">사용자 신고</button></li>';
	list += '<li class="_hql7s"><button class="_h74gn" id="btnUserBlock" onclick="userBlock(event)">이 사용자 차단하기</button></li>';
	list += '<li class="_hql7s"><button class="_h74gn" id="btnCancle" onclick="callRemoveDialog(event)">취소</button></li>';
	
	$("._cepxb").html(list);
	
	$("._o2wxh").on("click", function(){
		$("._o2wxh, ._hql7s").css("display","none");
		event.stopPropagation();
	});
...
function userReport(){
	$("._pfyik").css("display","none");
	$("._pfyik").remove();
	//모달창 불러오기
	var source = $("#modalReport").html();
	var report = Handlebars.compile(source);
	var data = {nickname : "${userVO.nickname}" , writer : "${login.nickname}"};
	var itemList = report(data);
	console.log(itemList);
	$("body").append(itemList);
	
	//modal창 보이기
	$("#myReportModal").css("display","block");
	
	//modal끄기 메서드-바깥부분
	$("#myReportModal").click(function(event){
...
function reportRegister(){
	...
	if(caption == null || typeof caption == "undefined" || caption.trim() == "") {
		alert("내용을 입력해주세요.");
		return false;
	}
	...
	$.ajax({
		type : 'post',
		url : '../admin/report/user/register',
		headers : {
