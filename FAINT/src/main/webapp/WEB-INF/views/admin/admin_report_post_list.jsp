            ...
            <div class="row">
                <div class="col-md-12 col-sm-12 col-xs-12 table-responsive">
					...
					<table id="posts-table" class="table table-tools table-striped">
						...
						<tbody>
							<c:forEach items="${ARPList}" var="ReportVO">
								<tr>
									...
									<c:choose>
										...
										<c:when test = "${ReportVO.cateid == 2}"><td>자해</td></c:when>
										<c:when test = "${ReportVO.cateid == 3}"><td>괴롭힘 및 따돌림</td></c:when>
										<c:when test = "${ReportVO.cateid == 4}"><td>약물 사용</td></c:when>
										<c:when test = "${ReportVO.cateid == 5}"><td>음란물</td></c:when>
										<c:when test = "${ReportVO.cateid == 6}"><td>폭력적인 이미지</td></c:when>
										<c:when test = "${ReportVO.cateid == 7}"><td>편파적 발언 및 상징</td></c:when>
									...
									<td><a href="/member/${ReportVO.nickname}">${ReportVO.nickname}</a></td>
									<td id = "reportReportid"><input type = 'hidden' value = "${ReportVO.reportid}">
									<a href="/member/${ReportVO.writer}">${ReportVO.writer}</a></td>
									...
									<td id = "reportPostid"><button value = "${ReportVO.postid}">게시물 상세보기</button></td>
								</tr>
							</c:forEach>
						</tbody>
					</table>
...
function reportRead(data){
	...
	var userid = ${login.id};
	var rReportid = $(data).siblings('#reportReportid').children("button").val();
	var rIsRead = $(data).siblings('#reportIsRead').children("input").val();

	if(id != rReportid && userid != rIsRead){
	  	$.ajax({
			type : 'post',
			url : '/admin/report/post/read/check',
			headers : {
	...
	$.getJSON("/admin/report/post/read/" + id, function(data){
		if(data.length!=0){
			var reportList="";         	
...
$(".pagination li a").on("click", function(event) {
	...
	var jobForm = $("#jobForm");
		...
		jobForm.attr("action", "<c:url value='/admin/report/post'/>").attr("method", "get");
		jobForm.submit();
});
</script>
